...
function StatCard({
  ...
  return (
    ...
    >
      ...
      {loading ? (
        <Skeleton className="h-7 w-14 mb-1 rounded" />
      ) : (
...
const ParentDashboard = () => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const { data: sitterData } = await supabase
          ...
          .select(
            "id, user_id, name, city, hourly_rate, rating_avg, is_verified, location_lat, location_lng",
          );
        if (sitterData) {
        ...
        if (bookingsRes.data) {
          const ids = [
            ...new Set(bookingsRes.data.map((b) => b.babysitter_id)),
          ];
          if (ids.length > 0) {
            ...
            sitterProfiles?.forEach((s) => {
              nameMap[s.user_id] = s.name;
            });
            setBookings(
              bookingsRes.data.map((b) => ({
                ...b,
                babysitter_name: nameMap[b.babysitter_id] ?? "Babysitter",
              })),
            );
          } else {
            setBookings(
  ...
  useEffect(() => {
    ...
    const source = activeSearch.trim()
      ? sitterLocations.filter(
          (s) =>
            s.city?.toLowerCase().includes(activeSearch.trim().toLowerCase()) ||
            s.name?.toLowerCase().includes(activeSearch.trim().toLowerCase()),
        )
      : sitterLocations;
    // Default center: first sitter if available, else Delhi (India)
    const center: [number, number] =
      source.length > 0 ? [source[0].lat, source[0].lng] : [28.6139, 77.209];
        ...
        <header className="hidden lg:flex items-center justify-between px-8 py-5">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative" ref={profileMenuRef}>
              ...
              {showProfileMenu && (
                <div
                  className="absolute right-0 top-12 w-52 rounded-2xl shadow-2xl py-1 z-[2000]"
                  style={{
                    background: "#0A1714",
                    border: `1px solid ${BORDER}`,
                  }}
        ...
        <div className="px-4 sm:px-6 lg:px-8 max-w-[1080px] pt-4">
                ...
                >
                  {activeSearch
                    ...
                    : sitterLocations.length > 0
                      ? `${sitterLocations.length} sitter${sitterLocations.length !== 1 ? "s" : ""} on the map`
                      : "Discover verified sitters in your area"}
                </p>
              </div>
              <Link to="/babysitters">
                <button
                  className="inline-flex items-center gap-1.5 font-semibold rounded-full px-4 py-2 text-sm text-white transition-all hover:opacity-90"
                  style={{ background: TEAL }}
                >
                  <Search size={13} />
                  Search
            ...
            {[
              {
                ...
                icon: Search,
                label: "Find a Sitter",
                desc: "Search & compare verified babysitters",
                color: TEAL,
              },
              {
                to: "/parent/inbox",
                icon: MessageCircle,
                label: "Messages",
                desc: "Chat directly with your sitters",
                color: "#60A5FA",
              },
              {
                to: "/post-job",
                icon: BookOpen,
                label: "Post a Job",
                desc: "Post a job and let sitters come to you",
                color: "#A78BFA",
              },
              ...
              <Link
                key={label}
                to={to}
                className="group rounded-2xl p-4 flex items-center gap-4 transition-all hover:brightness-110"
                style={{ background: CARD, border: `1px solid ${BORDER}` }}
              >
                <div
          ...
          >
            <div className="px-5 pt-5 pb-0">
              ...
              <div
                className="flex gap-1 rounded-xl p-1 mb-1"
                ...
                {(["upcoming", "past"] as const).map((tab) => {
                  ...
                  return (
                    ...
                    >
                      {tab}{" "}
                      <span
                        className="ml-1 text-[11px] px-1.5 py-0.5 rounded-md font-semibold"
                        style={{
                          background: isActive
                            ? "rgba(61,190,181,0.12)"
            ...
            <div className="p-5">
              {loading ? (
                ...
                <div className="text-center py-14">
                  ...
                  >
                    {activeTab === "upcoming"
                      ? "Find a trusted babysitter and book your first session!"
                      : "Your completed bookings will show up here"}
                  </p>
                  {activeTab === "upcoming" && (
                    <Link to="/babysitters">
                      <button
                        className="flex items-center gap-2 font-semibold rounded-full px-6 py-2.5 text-sm text-white mx-auto"
                        style={{ background: TEAL }}
                      >
                        <Search size={14} /> Browse babysitters
                      </button>
                    </Link>
                  )}
                </div>
                        ...
                        <div className="flex items-center justify-between gap-4 flex-wrap">
                          <div className="flex items-center gap-3">
                            <div
                              ...
                              style={{ background: TEAL }}
                            >
                              {booking.babysitter_name?.charAt(0) ?? "B"}
                            </div>
                            <div>
                              <p className="font-semibold text-white text-sm">
                                {booking.babysitter_name}
                              </p>
                              <div className="flex items-center gap-2 mt-1 flex-wrap">
                                ...
                                {booking.total_price && (
                                  <span
                                    ...
                                    style={{ color: TEAL }}
                                  >
                                    SGD {Number(booking.total_price).toFixed(2)}
                                  </span>
                                )}
                              </div>
                            </div>
                          ...
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {booking.status === "pending" && (
                              <button
                                className="text-xs font-medium px-2 py-1 rounded-lg text-red-400 hover:bg-red-400/10 transition-colors"
                            ...
                            {booking.status === "completed" && (
                              <button
                                ...
                                onClick={() =>
                                  navigate(
                                    `/parent/review/${booking.id}?sitter=${booking.babysitter_id}`,
                                  )
                                }
                              >
                                <Star size={11} /> Review
                            ...
                            <Link
                              to={`/parent/inbox?with=${booking.babysitter_id}`}
                            >
                              <button
...
function BottomNav() {
  ...
  return (
    <div className="lg:hidden fixed bottom-0 inset-x-0 z-50">
      <div
        className="px-2 pb-[env(safe-area-inset-bottom)]"
          ...
          {items.map(({ icon: Icon, label, href }) => {
            ...
            return (
              <Link
                ...
                style={{ color: active ? TEAL : "rgba(255,255,255,0.35)" }}
              >
                <Icon size={20} />
