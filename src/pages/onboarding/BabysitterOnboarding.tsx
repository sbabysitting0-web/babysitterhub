...
const BabysitterOnboarding = () => {
  ...
  const [availability, setAvailability] = useState<{
    [d: number]: { start: string; end: string; enabled: boolean };
  }>({
  ...
  const saveProfile = async () => {
    ...
    Promise.resolve(
      supabase.rpc("ensure_user_record", { p_role: "babysitter" }),
    ).catch(() => {});
    toast({
      title: "Profile saved!",
      description: "Parents in your area can now find you.",
    });
    setLoading(false);
  };
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          background:
            "radial-gradient(ellipse, rgba(61,190,181,0.08) 0%, transparent 70%)",
        }}
      />
      ...
      <div className="w-full max-w-xl relative z-10">
        ...
        <div className="text-center mb-6">
          <Link
            to="/"
            className="inline-flex items-center gap-2.5 justify-center mb-4"
          >
              ...
              <React.Fragment key={s.label}>
                <div className="flex flex-col items-center gap-1">
                  <div
                    ...
                    style={{
                      borderColor: done || active ? TEAL : BDR,
                      background: done
                        ? TEAL
                        : active
                          ? "rgba(61,190,181,0.15)"
                          : "transparent",
                    }}
                  >
                    {done ? (
                      ...
                      <Icon
                        size={16}
                        style={{
                          color: active ? TEAL : "rgba(255,255,255,0.25)",
                        }}
                      />
                    )}
                  </div>
                  <span
                    ...
                    style={{ color: active ? TEAL : "rgba(255,255,255,0.25)" }}
                  >
                    {s.label}
                  </span>
                </div>
                {i < STEPS.length - 1 && (
                  <div
                    ...
                    style={{ background: i < step ? TEAL : BDR }}
                  />
                )}
              </React.Fragment>
            );
            ...
            {step === 0 && (
              <div className="space-y-5">
                ...
                <p className="text-sm text-white/40">
                  A photo makes parents 3× more likely to reach out.
                </p>
                <div className="flex flex-col items-center gap-3">
                  <div
                    ...
                    style={{
                      borderColor: TEAL,
                      background: "rgba(61,190,181,0.08)",
                    }}
                ...
                <div className="space-y-1.5">
                  ...
                  <Input
                    placeholder="e.g. Experienced babysitter & early childhood educator"
                    value={headline}
                ...
                <div className="space-y-1.5">
                  ...
                  <Input
                    placeholder="e.g. Jakarta, Manila"
                    value={city}
            ...
            {step === 1 && (
              <div className="space-y-5">
                ...
                <div className="space-y-1.5">
                  ...
                  <Textarea
                    placeholder="Tell parents about yourself, your experience and approach to childcare..."
                    rows={5}
                    value={bio}
                  ...
                  <div className="flex gap-2">
                    {["1", "2", "3", "4", "5+"].map((n) => (
                      <button
                        key={n}
                        type="button"
                        onClick={() => setMaxKids(n)}
                        className="flex-1 py-2.5 rounded-xl border text-sm font-medium transition-all"
                        style={{
                          borderColor: maxKids === n ? TEAL : BDR,
                          background:
                            maxKids === n
                              ? "rgba(61,190,181,0.15)"
                              : "transparent",
                          color: maxKids === n ? TEAL : "rgba(255,255,255,0.4)",
                        }}
                      >
                        {n}
                      </button>
                    ...
                    {AGE_GROUPS.map((ag) => {
                      ...
                      return (
                        <button
                          ...
                          type="button"
                          onClick={() =>
                            setAgeGroups(toggle(ageGroups, ag.label))
                          }
                          className="p-3 rounded-xl border text-left transition-all"
                          style={{
                            borderColor: active ? TEAL : BDR,
                            background: active
                              ? "rgba(61,190,181,0.1)"
                              : "transparent",
                          }}
            ...
            {step === 5 && (
              <div className="text-center space-y-5 py-6">
                ...
                <p className="text-white/40 max-w-xs mx-auto">
                  Your profile is live. Parents in your area can now find and
                  contact you.
                </p>
                <div className="flex items-center justify-center gap-6 text-sm text-white/40 pt-1">
                  {skills.length > 0 && (
                    <span>
                      <strong className="text-white">{skills.length}</strong>{" "}
                      skills
                    </span>
                  )}
                  {languages.length > 0 && (
                    <span>
                      <strong className="text-white">{languages.length}</strong>{" "}
                      languages
                    </span>
                  )}
                  {Object.values(availability).filter((v) => v.enabled).length >
                    0 && (
                    <span>
                      <strong className="text-white">
                        {
                          Object.values(availability).filter((v) => v.enabled)
                            .length
                        }
                      </strong>{" "}
                      days available
