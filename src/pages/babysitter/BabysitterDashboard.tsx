...
const SidebarLink = ({
  ...
  const cls =
    "flex items-center gap-3 px-4 py-2.5 text-sm rounded-xl transition-all w-full cursor-pointer";
  const style = active
    ? { background: "rgba(61,190,181,0.12)", color: TEAL }
...
const StatCard = ({
  ...
  <div
    className="rounded-2xl px-5 py-4 flex items-center gap-4 transition-all hover:brightness-110"
  ...
  >
    ...
    <div>
      <p className="text-2xl font-heading font-bold text-white leading-none">
        {value}
      </p>
...
/* Dark input helpers */
const fieldCls =
  "w-full rounded-xl px-3 py-2 text-sm text-white outline-none focus:ring-1 focus:ring-[#3DBEB5] transition-all";
const fieldStyle = {
...
const BabysitterDashboard = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (bookingsRes.data) {
        const parentIds = [
          ...new Set(bookingsRes.data.map((b) => b.parent_id)),
        ];
        ...
        setBookings(
          bookingsRes.data.map((b) => ({
            ...b,
            parent_name: nameMap[b.parent_id] ?? "Parent",
          })),
        );
      }
      setLoading(false);
  ...
  const saveProfile = async () => {
    ...
    const { error } = await supabase
      ...
      .update({
        ...
        years_experience: editProfile.years_experience,
    ...
    if (!error) {
      setProfile((prev) =>
        prev ? { ...prev, ...editProfile, is_verified: isComplete } : null,
      );
    ...
    <div className="min-h-screen" style={{ background: BG }}>
      ...
      >
        ...
        <div className="px-3 pb-5">
          <button
            ...
            className="flex items-center gap-3 px-4 py-2.5 text-sm rounded-xl transition-all w-full hover:text-red-400 text-left"
      ...
      <div className="lg:ml-[252px] min-h-screen pb-24 lg:pb-8">
        ...
        <header className="hidden lg:flex items-center justify-between px-8 py-5">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative" ref={profileMenuRef}>
              ...
              {showProfileMenu && (
                <div
                  className="absolute right-0 top-12 w-52 rounded-2xl shadow-2xl py-1 z-50"
                  style={{
                    background: "#0A1714",
                    border: `1px solid ${BORDER}`,
                  }}
                >
                  ...
                  <div style={{ borderTop: `1px solid ${BORDER}` }}>
                    <button
                      ...
                      className="flex items-center gap-2.5 px-4 py-2.5 text-sm text-red-400 w-full text-left"
                    >
        ...
        <div className="px-4 sm:px-6 lg:px-8 max-w-[1080px] pt-4">
          ...
          {profile &&
            !!!(
              ...
              >
                ...
                >
                  Profile incomplete. Fill in bio, city and hourly rate to
          ...
          {/* Tab bar */}
          <div
            className="flex mb-5 gap-1 rounded-xl p-1"
            ...
            {TABS.map((tab) => (
              <button
                ...
                style={{
                  ...
                  boxShadow:
                    activeTab === tab ? "0 1px 4px rgba(0,0,0,0.3)" : "none",
                }}
              >
                {tab}
            ...
            <div className="space-y-3">
              {loading ? (
                ...
                >
                  ...
                  >
                    They'll appear here once parents book you.
                  </p>
                </div>
              ) : (
                bookings.map((booking) => {
                  ...
                  return (
                    <div
                      key={booking.id}
                      className="rounded-2xl p-5 flex items-start justify-between gap-4 flex-wrap hover:brightness-110 transition-all"
                      style={{
                        background: CARD,
                        border: `1px solid ${BORDER}`,
                      }}
                    >
                      <div className="flex items-center gap-3">
                        <div
                          ...
                          style={{ background: "rgba(61,190,181,0.12)" }}
                        >
                        ...
                        <div>
                          ...
                          {booking.total_price && (
                            ...
                            >
                              <DollarSign size={11} />
                              {
                                getCurrencyForCity(profile?.city ?? "").code
                              }{" "}
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        {booking.status === "pending" && (
                          <>
                            <Button
                              size="sm"
                              className="h-8 text-xs rounded-full px-4 text-white"
                              style={{ background: TEAL }}
                              onClick={() =>
                            ...
                            <Button
                              variant="outline"
                              size="sm"
                              className="h-8 text-xs rounded-full px-4 text-red-400 border-red-900 bg-transparent hover:bg-red-400/10"
                              onClick={() =>
                        ...
                        {booking.status === "confirmed" && (
                          <Button
                            variant="outline"
                            size="sm"
                            className="h-8 text-xs rounded-full px-4 bg-transparent border-white/10 text-white/60 hover:text-white"
                            onClick={() =>
                        ...
                        <Link to={`/parent/inbox?with=${booking.parent_id}`}>
                          <Button
                            variant="outline"
                            size="sm"
                            className="h-8 text-xs rounded-full px-3 bg-transparent border-white/10 text-white/50 hover:text-white"
                          >
            ...
            >
              ...
              <div className="grid sm:grid-cols-2 gap-5">
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    type="number"
                    value={editProfile.years_experience ?? ""}
                    onChange={(e) =>
                      setEditProfile({
                        ...editProfile,
              ...
              <div className="flex items-center gap-3">
                <button
                  onClick={saveProfile}
                  disabled={saving}
                  className="flex items-center gap-2 font-semibold rounded-full px-6 py-2.5 text-sm text-white transition-all hover:opacity-90 disabled:opacity-50"
                  style={{ background: TEAL }}
                >
                ...
                {!!(
                  ...
                  <span
                    ...
                    style={{ color: TEAL }}
                  >
                    <TrendingUp size={13} /> Profile complete
                  </span>
                )}
              </div>
            </div>
            ...
            >
              <div className="mb-5">
                ...
                <p
                  ...
                  style={{ color: "rgba(255,255,255,0.35)" }}
                >
                  Toggle days on/off and adjust your working hours.
                </p>
              </div>
          ...
          {(
            ...
            return (
              <button
                ...
                style={{ color: isActive ? TEAL : "rgba(255,255,255,0.35)" }}
              >
                <Icon size={18} />
                {label}
              </button>
...
const AvailabilityManager = ({ userId }: { userId: string }) => {
  ...
  const save = async () => {
    ...
    await supabase
      ...
      .eq("babysitter_id", userId);
    if (rows.length > 0)
      await supabase.from("babysitter_availability").insert(rows);
    setSaving(false);
    toast({ title: "Availability updated!" });
  };

  const timeCls =
    "text-xs rounded-lg px-2 py-1.5 outline-none focus:ring-1 focus:ring-[#3DBEB5] transition-all";
  const timeStyle = {
    background: "rgba(255,255,255,0.06)",
    border: `1px solid ${BORDER}`,
    color: "#fff",
    ...
    <div className="space-y-2">
      {DAYS.map((day, i) => (
        <div
          ...
          style={{
            borderColor: availability[i].enabled
              ? "rgba(61,190,181,0.3)"
              : BORDER,
            background: availability[i].enabled
              ? "rgba(61,190,181,0.05)"
