...
const HowItWorksSection = () => {
  ...
  useEffect(() => {
    ...
    const cards = Array.from(
      container.querySelectorAll<HTMLElement>(".step-card"),
    );
  ...
  return (
    ...
    >
      ...
      <div
        className="absolute left-0 bottom-12 pointer-events-none"
        style={{
          width: "50px",
          height: "100px",
          background:
            "linear-gradient(90deg, rgba(61,190,181,0.08) 0%, transparent 100%)",
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div ref={titleRef} className="text-center max-w-2xl mx-auto mb-16">
          ...
          <h2 className="text-3xl md:text-4xl font-heading font-extrabold text-white mb-4">
            Hire a babysitter online in 3 simple steps
          </h2>
          <p className="text-white/40 text-base">
            No complicated forms. Just search, connect, and book.
          </p>
        </div>
        ...
        <div ref={cardsRef} className="grid md:grid-cols-3 gap-6 mb-12">
          {steps.map((step, idx) => {
            ...
            return (
              <div
                key={idx}
                className="step-card relative bg-[#0E1E1A] border border-white/6 rounded-3xl p-8 hover:border-teal/30 hover:shadow-xl hover:shadow-teal/5 transition-all duration-300 group"
              >
                <div className="absolute top-7 right-7 text-5xl font-extrabold text-white/4 font-heading leading-none select-none">
                  {step.number}
                </div>
        ...
        <div className="text-center">
          <Link
            to="/babysitters"
            className="inline-flex items-center gap-2 bg-teal text-white font-semibold px-8 py-4 rounded-full hover:bg-teal/90 hover:shadow-lg hover:shadow-teal/25 hover:-translate-y-0.5 transition-all text-sm"
          >
